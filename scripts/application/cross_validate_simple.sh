...
for j in $(seq 0 $((REPS-1))); do
  for i in $(seq 0 $((SPLITS-1))); do
    ...
    --outcome=continuous --flip-outcome --select-latent-dim=4 --repertoire-latent-dim=4 \
    --select-embed-layer-dim=8 --posterior-rank=-1 --select-posterior-rank=-1 \
    --selection-channels=8 --encoder-channels=8 \
    --conv-kernel=3 --selection-conv-kernel=3 --encoder-conv-kernel=3 \
    --no-attention --n-selection-layers=1 --pos-encode --blosum \
    --encoder-no-attention --linear-cnn --sum-pool \
